/**
 * Created by Nofear on 3/15/2019.
 */

(function () {
    cc.MiniPokerTopView = cc.Class({
        "extends": cc.PopupBase,
        properties: {
            miniPokerTopListView: cc.MiniPokerTopListView,
            toggleContainer: cc.ToggleContainer
        },

        onLoad: function () {
            this.animation = this.node.getComponent(cc.Animation);
            this.node.zIndex = cc.NoteDepth.POPUP_TAIXIU;
        },
        onChangeTab(event, type) {
            // console.log('change Tab', event.node.name)
            this.getTop(parseInt(event.node.name));
        },
        onEnable: function () {
            var self = this;
            var delay = 0.2;
            cc.director.getScheduler().schedule(function () {
                // self.getTop();
                self.toggleContainer.toggleItems[0].checked = true
                self.getTop(1)
            }, this, 1, 0, delay, false);

            this.animation.play('openPopup');
        },

        getTop: function (type = 1) {
            var mpGetTopCommand = new cc.MPGetTopCommand;
            mpGetTopCommand.execute(this, type);
        },

        onMPGetTopResponse: function (response) {
            var list = response;
            //var list = slotsHistoryListData;
            if (list !== null && list.length > 0) {
                this.miniPokerTopListView.resetList();
                this.miniPokerTopListView.initialize(list);
            }
        },

        closeClicked: function () {
            this.miniPokerTopListView.resetList();
            this.animation.play('closePopup');
            var self = this;
            var delay = 0.12;
            cc.director.getScheduler().schedule(function () {
                self.animation.stop();
                cc.MiniPokerController.getInstance().destroyTopView();
            }, this, 1, 0, delay, false);
        },
    });
}).call(this);
