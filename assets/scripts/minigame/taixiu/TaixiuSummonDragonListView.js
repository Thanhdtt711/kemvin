...
(function () {
    cc.TaixiuSummonDragonListView = cc.Class({
        "extends": cc.ListView,
        properties: {
        ...
        initialize: function (messages) {
            ...
            for (var i = 0; i < spawnCountReal; ++i) {
                ...
                item.getComponent(cc.TaixiuSummonDragonItem).updateItem(this.messages[i], i, this.iconTop);
        ...
        update: function (dt) {
            ...
            //             var item = items[i].getComponent(cc.TaixiuSummonDragonItem);
            //             var itemId = item.itemID - items.length; // update item id
            //             item.updateItem(this.messages[itemId], itemId, this.iconTop);
            //         }
            //     } else {
            //         // if away from buffer zone and not reaching bottom of content
            //         if (viewPos.y > buffer && items[i].y - offset > -this.content.height) {
            //             items[i].y = (items[i].y - offset);
            //             item = items[i].getComponent(cc.TaixiuSummonDragonItem);
