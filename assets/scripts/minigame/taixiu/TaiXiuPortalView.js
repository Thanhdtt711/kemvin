...
(function () {
    cc.TaiXiuPortalView = cc.Class({
        ...
        updateInfo: function (sessionInfo) {
            ...
            //     && sessionInfo.CurrentState !== this.currentState
            //     && sessionInfo.CurrentState === cc.TaiXiuState.RESULT) {
            //     if (sessionInfo.Result.Dice1 + sessionInfo.Result.Dice2 + sessionInfo.Result.Dice3 > 10) {
            //         this.animationTaiXiu.play('taiWin');
            //     } else {
            //         this.animationTaiXiu.play('xiuWin');
            //     }
            // } else if (sessionInfo.CurrentState !== this.currentState) {
            //     this.animationTaiXiu.stop();
            //     this.nodeParentTai.active = true;
            //     this.nodeParentXiu.active = true;
            // }
            ...
            //         this.lbTimer.node.color = cc.Color.YELLOW;
            //         break;
            ...
            if (response.M !== undefined && response.M.length > 0) {
                ...
                switch (m.M) {
                    ...
                    case cc.MethodHubOnName.WIN_RESULT:
                        ...
                        break;
                    case cc.MethodHubOnName.GET_REFUND_ACCOUNT:
                        var data = m.A[0];
                        var waitTime = taiXiuConfig.TIME_WAIT_DICE_ANIMATION;
                        //dang bat che do Nan
                        cc.director.getScheduler().schedule(function () {
                            //play fx win
                            cc.TaiXiuController.getInstance().playEffectWin(data.Refund);
                            //update lai balance
                            cc.BalanceController.getInstance().updateRealBalance(data.Balance);
                            cc.BalanceController.getInstance().updateBalance(data.Balance);
                        }, this, 0, 0, waitTime, false);
