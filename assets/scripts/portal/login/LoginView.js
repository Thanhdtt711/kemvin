...
//var secretkey = '5f4dcc3b5aa765d61d8327deb882cf99';
    ...
    cc.LoginView = cc.Class({
        ...
        onEnable: function () {
            ...
            if (tool.getItem('@rememberPassword') !== null) {
                if (tool.getItem('@rememberPassword') === 'true') {
                    // var userPassData = "";
                    // var cipherPassText = tool.getItem('@password');
                    // if (this.tryParseJSONObject(cipherPassText)) {
                    //     userPassData = JSON.parse(encrypt.decrypt(cipherPassText, secretkey, 256));
                    // }
                    this.editBoxUsername.string = tool.getItem('@username').toString();
                    //this.editBoxPassword.string = userPassData.toString();
        ...
        callLogin: function () {
            ...
            if (this.toggleRememberPass.isChecked) {
               // var dataPassString = JSON.stringify(this.password);
                //var encryptedPass = encrypt.encrypt(dataPassString, secretkey, 256);
                cc.Tool.getInstance().setItem('@username', this.username);
                //cc.Tool.getInstance().setItem('@password', encryptedPass);
